...
public class WeaponState : State
{
    ...
    private GameObject mainWeaponReference;
    ...
    public void UseItem()
    {
        if (Time.time >= nextAttackAllowedTime && playerMovement.isDashing == false)
        {
            ...
            StartCoroutine(NextAttackCombo());
        }
        
    }

    IEnumerator NextAttackCombo()
    {
        yield return new WaitForSeconds(attackCooldown);
        stateMachineManager.setNewState(nextAttackState);
    }
    ...
    void swordAttack()
    {
        //animations
        CreateSlash();
        pivotPoint.GetComponent<Animator>().SetTrigger(animatorTriggerName);
